...
parser.add_argument('-cfl', '--complex_fusion_length', type=int, help='Max distance to merge breakpoints (default=1000)', default=1000)
...
def parse_vcf(vcf, vcf_output, info_output, pdf, full_vcf):
    ...
    with open(vcf, "r") as vcf, open(info_output, "w") as fusion_output, PdfPages(pdf) as output_pdf, open(vcf_output, "w") as vcf_output:
        ...
        for complex_sv, reads in complex_reads.items():
            ...
            if piece_length<args.complex_fusion_length:
                bnd_info={}
                BND=1
